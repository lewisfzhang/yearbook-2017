<?php
	include 'users.php'; //user and password
	$user = $_SERVER['PHP_AUTH_USER'];
	$pass = $_SERVER['PHP_AUTH_PW'];
    //auth
	$validated = ($user==$valid_user)&&($pass==$valid_password);
	if($validated) {
		$url = $_POST["id"]; //retreive the POSTed info
		$file_db = new PDO('sqlite:../quotations.sqlite3');
		$file_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$quotation = $_POST["quotation"];
		$quotation = trim(preg_replace('/\s+/', ' ', $quotation));
		$find = "UPDATE quotations SET quotation=:quotation, processedTeacher=2, processedStudent=2 WHERE url=:url"; //update to say that it was accepted
